...
public final class LlmProviderRegistry {
  ...
  public LlmProviderRegistry(Map<String, Function<ProviderSettings, Ports.LlmProvider>> factories) {
  ...
  public Ports.LlmProvider select(Map<String, String> env) {
    ...
    if (!env.getOrDefault("LLM_FALLBACK_PROVIDERS", "").isBlank())
      throw new IllegalArgumentException(
          "Automatic fallback is disabled in V1; clear LLM_FALLBACK_PROVIDERS and explicitly select LLM_PROVIDER");
    ...
    if (factory == null)
      throw new IllegalArgumentException(
          "Unknown LLM_PROVIDER '"
              + key
              + "'; supported: "
              + String.join(", ", new TreeSet<>(factories.keySet())));
    var settings = ProviderSettings.from(key, env);
    settings.validateLive();
