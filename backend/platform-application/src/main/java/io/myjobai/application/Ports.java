...
public final class Ports {
  ...
  public interface Outbox {
    AsyncJob enqueue(
        UUID user, String type, Map<String, String> payload, String idempotencyKey, Instant now);
  ...
  public record Performance(
      String label, long applications, long responses, long interviews, long offers) {}

  public record AnalyticsSummary(
      Map<String, Long> lifecycle,
      List<Performance> sources,
      List<Performance> resumes,
      Map<String, Long> outreach,
      long aiRequests,
      long cacheHits,
      long inputTokens,
      long outputTokens,
  ...
  public interface Analytics {
    AnalyticsSummary summary(UUID user);

    List<Map<String, Object>> activity(UUID user, int limit);
  }
  ...
  public record LlmRequest(
      ...
      Tier tier,
      String system,
      Map<String, Object> untrustedData,
  ...
  public interface LlmProvider {
    ...
    Completion generate(LlmRequest request);
  }

  public interface Intelligence {
    Completion generate(UUID user, LlmRequest request);
  }

  public record SearchResult(String title, String url, String snippet) {}

  public interface WebResearch {
    List<SearchResult> search(String query);

    String publicPage(String url);
  }

  public record CaptureInput(
      String url, String title, String text, String company, Map<String, Object> metadata) {}

  public interface PageCaptures {
    DiscoveredJob extract(CaptureInput input);
  }

  public record ContactEvidence(String label, String value, Contact.Type type, String sourceUrl) {}

  public interface RecruiterResearch {
    List<ContactEvidence> find(Opportunity.Job job);
  }

  public record SourceCapabilities(
      boolean discovery, boolean jobDetails, boolean recruiter, String accessMethod) {}
  ...
  public interface JobSourceConnector {
    ...
    List<DiscoveredJob> discoverJobs(DiscoveryCriteria criteria);
  ...
  public record ApprovedMail(
      Outreach.Approval approval,
      Outreach.Version version,
      byte[] attachment,
      String mailboxAccessToken,
      String mailboxAddress) {
    @Override
    public String toString() {
      return "ApprovedMail[approval=" + approval.id() + ", credentials=redacted]";
    }
  }
  ...
  public interface MailboxProvider {
    ...
    SendResult send(ApprovedMail message);
  }
  ...
  public record Mailbox(
    ...
    @Override
    public String toString() {
      return "Mailbox[user=" + userId + ", provider=" + provider + ", credentials=redacted]";
    }
  }

  public interface Mailboxes {
    Optional<Mailbox> find(UUID user);
  ...
  public interface Connectors {
    List<ConnectorConfig> list(UUID user);

    void save(ConnectorConfig config);

    List<ConnectorConfig> due(Instant now);

    void schedule(UUID id, Instant nextRun);
