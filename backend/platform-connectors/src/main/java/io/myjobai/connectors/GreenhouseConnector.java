...
public final class GreenhouseConnector extends FeedConnector {
  public GreenhouseConnector(Function<String, String> fetch, JsonMapper json, Clock clock) {
    ...
    for (var node : payload.path("jobs")) {
      ...
      jobs.add(
          job(
              ...
              node.path("company_name").asText(board),
              location,
              node.path("content").asText(),
              location.toLowerCase().contains("remote"),
              "",
              node.path("absolute_url").asText(),
              node.path("first_published").asText(node.path("updated_at").asText())));
    }
    return jobs;
  }
}
