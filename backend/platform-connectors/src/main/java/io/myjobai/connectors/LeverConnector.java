...
public final class LeverConnector extends FeedConnector {
  public LeverConnector(Function<String, String> fetch, JsonMapper json, Clock clock) {
  ...
  protected List<Ports.DiscoveredJob> parse(String board, JsonNode payload) {
    ...
    for (var node : payload) {
      ...
      StringBuilder content =
          new StringBuilder(
              node.path("descriptionPlain").asText(node.path("description").asText()));
      for (var item : node.path("lists"))
        content
            .append(' ')
            .append(item.path("text").asText())
            .append(' ')
      ...
      jobs.add(
          job(
              ...
              board,
              categories.path("location").asText(""),
              content.toString(),
              node.path("workplaceType").asText().equals("remote"),
              categories.path("commitment").asText(""),
              node.path("hostedUrl").asText(),
              node.has("createdAt")
                  ? Instant.ofEpochMilli(node.path("createdAt").asLong()).toString()
                  : ""));
    }
    return jobs;
  }
