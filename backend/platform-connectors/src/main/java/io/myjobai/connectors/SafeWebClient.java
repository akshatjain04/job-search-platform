...
public final class SafeWebClient implements AutoCloseable {
                ...
                new DnsResolver() {
                  public InetAddress[] resolve(String host) throws UnknownHostException {
                    ...
                    for (var address : addresses)
                      if (!isPublic(address))
                        throw new UnknownHostException("Non-public destination is prohibited");
                    return addresses;
                  }
  ...
  public static boolean isPublic(InetAddress a) {
    ...
    int first = b[0] & 255;
    if (b.length == 16)
      return (first & 254) != 252
          && !(first == 32 && (b[1] & 255) == 1 && (b[2] & 255) == 13 && (b[3] & 255) == 184);
    int second = b[1] & 255;
    return first != 0
        && first < 224
  ...
  public String get(String url, Map<String, String> headers) {
    ...
    request.setHeader("User-Agent", "MyJobAI/0.1 (+user-initiated research)");
    ...
    try {
      return client.execute(
          ...
          response -> {
            ...
            try (var input = response.getEntity().getContent()) {
              byte[] bytes = input.readNBytes(2000001);
              if (bytes.length > 2000000)
                throw new IntegrationException(
                    "SOURCE_TOO_LARGE", "Source exceeds 2 MB", false, false);
      ...
      throw new IntegrationException(
          "SOURCE_CONNECTION",
          "Public source could not be fetched; redirects and non-public addresses are blocked",
          true,
          false);
    }
  }
