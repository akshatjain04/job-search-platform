...
public final class ApplicationLifecycle {
    ...
    public enum State { DISCOVERED, SAVED, SHORTLISTED, PREPARING, READY_TO_APPLY, APPLIED, OUTREACH_PREPARED, OUTREACH_SENT, REPLIED, INTERVIEW, OFFER, REJECTED, WITHDRAWN }
    private static final Map<State,Set<State>> TRANSITIONS = Map.ofEntries(
        ...
        Map.entry(State.INTERVIEW,Set.of(State.OFFER)), Map.entry(State.OFFER,Set.of(State.WITHDRAWN)));
    public static void requireTransition(State from, State to) {
        boolean terminal = from == State.REJECTED || from == State.WITHDRAWN;
        if (terminal || from == to || !(to == State.WITHDRAWN || to == State.REJECTED || TRANSITIONS.getOrDefault(from,Set.of()).contains(to))) throw DomainException.conflict("Illegal application transition: "+from+" → "+to);
    }
    public record Application(UUID id, UUID userId, UUID jobId, State state, Instant createdAt, Instant updatedAt) {}
    public record Event(UUID id, UUID userId, UUID applicationId, State from, State to, String note, Instant at) {}
}
