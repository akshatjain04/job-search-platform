...
public final class Normalization {
  ...
  public static String url(String raw) {
                ...
                .filter(
                    p ->
                        !p.toLowerCase(Locale.ROOT)
                            .matches("(?:utm_[^=]*|ref|source|trk|trackingid|fbclid|gclid)=.*"))
                .sorted()
                .toList();
    ...
    return uri.getScheme().toLowerCase(Locale.ROOT)
        ...
        + path
        + (retained.isEmpty() ? "" : "?" + String.join("&", retained));
  }
  ...
  public static boolean sameJob(Opportunity.Job a, Opportunity.Job b) {
    ...
    return Math.abs(ChronoUnit.DAYS.between(a.postedAt(), b.postedAt())) <= 45
