...
public final class DocumentResumeParser implements Ports.ResumeParser {
  static {
    ...
    ZipSecureFile.setMaxEntrySize(20 * 1024 * 1024);
    ZipSecureFile.setMaxTextSize(500000);
  }

  public String extract(byte[] bytes, String mediaType) {
    if (bytes.length < 4 || bytes.length > 10 * 1024 * 1024)
      throw DomainException.invalid("Resume file size is invalid");
    try {
      String result;
      if (mediaType.equals("application/pdf")) {
        if (!new String(bytes, 0, 4, StandardCharsets.US_ASCII).equals("%PDF"))
          throw DomainException.invalid("File is not a PDF");
        try (var document = Loader.loadPDF(bytes)) {
          if (document.isEncrypted() || document.getNumberOfPages() > 30)
            throw DomainException.invalid(
                "Encrypted resumes or resumes longer than 30 pages are unsupported");
      ...
      if (result.isBlank() || result.length() > 100000)
        throw DomainException.invalid(
            "No usable resume text, or extracted text exceeds 100000 characters");
      return result;
    } catch (DomainException e) {
      throw e;
    } catch (Exception e) {
      throw DomainException.invalid(
          "Resume parsing failed: unsupported, corrupt or protected document");
    }
  }
}
