...
2. Read [references/review-boundaries.md](references/review-boundaries.md) and [references/evidence-ledger.md](references/evidence-ledger.md). Create a criterion-by-criterion evidence ledger from the current acceptance document, not a fixed count in this skill. From the repository root run `node skills/job-search-platform-repair/scripts/validate.mjs --root=. --static`. If installed elsewhere, invoke that installed script using its absolute path and pass the checkout's absolute path as `--root`.
3. Run the same validation helper with `--full`: all Java deployables, both browser clients, root script tests, empty PostgreSQL/Testcontainers migrations, Docker image/Compose startup, readiness/smoke, Playwright and persistence across restart. It requires Java 21, Maven, Node 22, Linux Docker/Compose and browser system libraries; see `docs/testing.md`. It uses explicit demo mode and leaves containers running for inspection. Read every failing test/report. Install documented build prerequisites when authorized; never skip failures to obtain green output.
